...
%     decision(decision==0) = -1;
    ...
    stats(j).fp = Tfp;
    stats(j).fn = Tfn;
    stats(j).acc = (Ttp+Ttn)/(Ttp+Ttn+Tfp+Tfn);%正确率均值
    stats(j).ppv = Ttp/(Ttp+Tfp);%分为正例的示例中实际为正例的比例 精度
    stats(j).sens = Ttp/(Ttp+Tfn);%真正类率 灵敏度 召回率
    stats(j).spec = Ttn/(Tfp+Ttn);%真负类率 所有负例中被分对的比例 特效度
