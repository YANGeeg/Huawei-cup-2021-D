...
if flags.do_training && flags.do_svmparam
    ...
    if isempty(params.c_range) % defaults
        params.c_range = -10:1:10;
    end
    if isempty(params.g_range)
        params.g_range = -10:1:10;
    end

    % cross-validation for parameter search
    for c = params.c_range
        for g = params.g_range
            svm_model = svmtrain(training_labels, training_data,[quietswitch '-t ' num2str(params.kernel) ' -c ' num2str(2^c) ' -g ' num2str(2^g) ' -v ' num2str(params.cvfolds)]);
...
if flags.do_training
    svm_model = svmtrain(training_labels,training_data,[quietswitch '-t ' num2str(params.kernel) ' -c ' num2str(2^outputs.c) ' -g ' num2str(2^outputs.g) ' -b 1']);
else
    svm_model = params.svm_model;
end

%% evaluate SVM
[predicted_labels acc prob_estimates] = svmpredict(testing_labels,testing_data,svm_model,'-b 1');
